  ...
  return (
    <div className="min-h-screen bg-[hsl(36,100%,97%)]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-orange-900 mb-2">Centres by State</h1>
          <p className="text-orange-700">Browse One Stop Centres in your state</p>
        </div>

        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {Array.from({ length: 12 }).map((_, i) => (
              <div key={i} className="bg-white rounded-2xl p-5 border border-orange-100 animate-pulse h-24" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {data?.states.map((s) => (
              <Link
                ...
                className="bg-white rounded-2xl p-5 border border-orange-100 shadow-sm hover:shadow-md hover:border-orange-300 transition-all flex items-center justify-between group"
              >
                <div>
                  <div className="font-semibold text-orange-900 group-hover:text-orange-700 transition-colors">{s.state}</div>
                  <div className="flex items-center gap-1 mt-1 text-sm text-orange-500">
                    <MapPin className="w-3.5 h-3.5" />
                    {s.count} {s.count === 1 ? "centre" : "centres"}
                  </div>
                </div>
                <div className="bg-orange-50 text-orange-700 font-bold text-lg w-10 h-10 rounded-xl flex items-center justify-center group-hover:bg-orange-100 transition-colors">
                  {s.count}
                </div>
              </Link>
            ))}
