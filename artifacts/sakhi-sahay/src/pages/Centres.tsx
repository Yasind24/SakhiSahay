  ...
  return (
    <div className="min-h-screen bg-[hsl(350,100%,98%)]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-rose-900 mb-1">Find a Centre</h1>
        ...
        {/* Filters */}
        <div className="bg-white rounded-2xl border border-rose-100 shadow-sm p-4 mb-6 space-y-3">
          {/* Search */}
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-rose-200 rounded-xl text-sm focus:outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-100 bg-[hsl(350,100%,99%)] transition-all"
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <select
              ...
              className="flex-1 border border-rose-200 rounded-xl px-3 py-2.5 text-sm text-rose-900 focus:outline-none focus:border-rose-500 bg-[hsl(350,100%,99%)] cursor-pointer"
            >
            ...
            {hasFilters && (
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2.5 text-sm text-rose-600 border border-rose-200 rounded-xl hover:bg-rose-50 transition-colors whitespace-nowrap"
              >
        ...
        {isLoading ? (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {Array.from({ length: 12 }).map((_, i) => (
              <div key={i} className="bg-white rounded-2xl p-5 border border-rose-100 animate-pulse h-36" />
            ))}
          </div>
        ) : data?.data.length === 0 ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
              {data?.data.map((osc) => (
                <Link
                  ...
                  className="bg-white rounded-2xl p-5 border border-rose-100 shadow-sm hover:shadow-md hover:border-rose-300 transition-all group block"
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1 min-w-0">
                      <span className="inline-block text-xs font-semibold bg-rose-100 text-rose-700 px-2 py-0.5 rounded-full mb-1.5">
                        {osc.state}
                      </span>
                      <h3 className="font-bold text-rose-900 text-base leading-snug group-hover:text-rose-700 transition-colors">
                        {osc.district}
                      </h3>
                    </div>
                    <span className="text-xs text-rose-300 font-mono ml-2 mt-1">#{osc.id}</span>
                  </div>

                  {osc.name && (
                    <p className="text-sm text-rose-700 mb-2 flex items-center gap-1.5">
                      <span className="w-4 h-4 inline-flex items-center justify-center text-rose-400 shrink-0">👤</span>
                      <span className="truncate">{osc.name}</span>
                    </p>
                  )}

                  {osc.address && (
                    <p className="text-xs text-rose-500 line-clamp-2 flex items-start gap-1.5">
                  ...
                  <div className="mt-3 pt-3 border-t border-rose-50 flex items-center justify-between">
                    <span className="text-xs text-rose-400 truncate">{osc.email}</span>
                    <span className="text-rose-500 group-hover:text-rose-700 transition-colors text-xs font-medium">Details →</span>
                  </div>
                </Link>
              ))}
            </div>
            ...
            {data && data.totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 flex-wrap">
                <button
                  ...
                  disabled={page === 1}
                  className="px-4 py-2 rounded-xl border border-rose-200 text-sm font-medium text-rose-700 hover:bg-rose-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
                  ← Prev
                </button>
